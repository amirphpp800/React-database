...
const IpList: React.FC<{
    ...
    if (list.length === 0) {
        return <div className="text-center text-gray-400 py-8">{t('noItemsToShow')}</div>;
    }

    return (
        <ul className="space-y-2 p-2 sm:p-4" dir="ltr">
            {list.map(ipInfo => (
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                                ...
                                aria-label={t('copyIpAriaLabel', { ip: ipInfo.address })}
                            >
                            ...
                            <button
                                ...
                                aria-label={t('deleteIpAriaLabel', { ip: ipInfo.address })}
                            >
...
const CountryCard: React.FC<CountryCardProps> = ({ country, ips, onDeleteIp, onToggleStatus, onDeleteSold }) => {
  ...
  const countryName = lang === 'fa' ? country.name : country.name_en;
    ...
    <div className="bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 shadow-xl overflow-hidden flex flex-col transition-all duration-300 hover:border-white/20">
      <header className="p-4 bg-white/5 border-b border-white/10 flex flex-wrap justify-between items-center gap-2">
        <div className="flex items-center gap-3">
          <img src={`https://flagcdn.com/w40/${country.iso.toLowerCase()}.png`} width="32" alt={t('flagAlt', { countryName })} className="rounded-sm" />
          <h3 className="text-xl font-bold text-white tracking-wide">{countryName}</h3>
        </div>
        <button
        ...
        >
          <TrashIcon className="w-4 h-4" />
          <span>{t('deleteSold')}</span>
        </button>
      </header>
      
      <div className="border-b border-white/10 px-2 flex justify-between items-center">
        <div className="flex">
            <TabButton title={t('ipv4')} count={ipv4s.length} isActive={activeTab === 'IPv4'} onClick={() => setActiveTab('IPv4')} />
            <TabButton title={t('ipv6')} count={ipv6s.length} isActive={activeTab === 'IPv6'} onClick={() => setActiveTab('IPv6')} />
        </div>
        <button
            onClick={handleCopy}
            disabled={activeList.length === 0}
            className="flex items-center gap-2 mx-2 px-3 py-1.5 bg-blue-500/20 text-blue-300 rounded-md text-sm font-medium hover:bg-blue-500/40 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
            {isCopied ? <CheckIcon className="w-4 h-4 text-green-400" /> : <CopyIcon className="w-4 h-4" />}
            <span>{isCopied ? t('copied') : t('copyList')}</span>
        </button>
      </div>
